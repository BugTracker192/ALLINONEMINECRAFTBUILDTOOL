...
def make_fixture(width: int = 24, height: int = 12, length: int = 24) -> BuildDocument:
    ...
    region = BuildRegion("Main", bounds.min, bounds.dimensions, bounds, tuple(item.canonical_state for item in palette))
    return BuildDocument("1.1.0", "bench_" + source_hash[:20], source, {}, bounds, bounds.min, palette, [region], blocks, region_blocks={"Main": dict(blocks)})
...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path, default=Path("var/reports/snowflake-cpu-render-benchmark.json"))
    ...
    try:
        ...
        iso, iso_seconds = timed(lambda: renderer.render(args.output_root, camera=CameraSpec.preset("isometric_ne"), size=size, mode="textured" if pack else "flat", name="benchmark_iso"))
        north, north_seconds = timed(lambda: renderer.render(args.output_root, camera=CameraSpec.preset("north"), size=size, mode="textured" if pack else "flat", name="benchmark_north"))
        layer, layer_seconds = timed(lambda: renderer.render_slice(args.output_root, axis="y", minimum=1, pixels_per_block=8, mode="textured" if pack else "flat", name="benchmark_layer"))
    finally:
        if pack:
            pack.close()
    ...
    report = {
        ...
        "renders": [
            {"name": "isometric", "seconds": iso_seconds, "png": str(iso.png_path), "diagnostics": iso.diagnostics},
            {"name": "north", "seconds": north_seconds, "png": str(north.png_path), "diagnostics": north.diagnostics},
            {"name": "layer_y_1", "seconds": layer_seconds, "png": str(layer.png_path), "diagnostics": layer.diagnostics},
        ],
    }
